...
| --- | --- | --- |
| Quality defaults | Set `Guaranteed Quality Every N Levels=0`, start a run, level repeatedly, record 10,000 promoted choices with a fixed test seed or large sample | Quality promotion is approximately 4%; 300 eligible rolls should average about 12 quality items, with a zero-result run having probability about 0.00048%. Available quality tiers follow normalized 70/20/8/2 weights. |
| Guaranteed quality set | Leave defaults (`Guaranteed Quality Every N Levels=5`, `Guaranteed Quality Choice Count=3`) and level through a fresh run | Every fifth level with a newly generated choice set shows 3 quality variants; their base white/green/red tier follows the current LevelUpChoices token progression, while quality tiers remain random. Unspent choices defer the queued set until the next generated set. |
...
| Guaranteed quality settings | Set the interval to `0`, then test intervals `1` and `5`; change the choice count from `1` through the upstream option count | Interval `0` produces no guaranteed sets; positive intervals queue one set per threshold; each generated set forces the configured number of quality choices, capped only when upstream returns fewer options. |
