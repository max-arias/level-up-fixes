...
| Quality chests | With Item Qualities installed, compare `Allow Quality Chests=false` and `true` while upstream removal is enabled | Default false removes `iscQualityChest1` and `iscQualityChest2` cards; true permits them to spawn. |
